...
def render_compose(source: Path, destination: Path, client: OrasClient) -> str:
    ...
    header = generated_header(source, destination, local)
...
def generated_header(source: Path, destination: Path, compose: CommentedMap) -> str:
    parent = destination.resolve().parent
    lines = [
        f"# Auto-generated by `{os.path.relpath(Path(__file__).resolve(), parent)}`",
        f"#   based on `{os.path.relpath(source.resolve(), parent)}` with resolved includes",
    ]
