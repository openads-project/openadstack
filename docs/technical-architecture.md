...
Most headless OpenADServices extend `ros2-service`. Specialized variants should only be used when an OpenADService requires display or accelerator access. `zenoh-router` is a concrete infrastructure Compose service rather than a base template.
