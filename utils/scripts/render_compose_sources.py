...
For each hidden `*.oci-overrides.yml`, merge OCI sources with local overrides,
derive service names from `NAMESPACE` and `NAME`, preserve replaced values as
comments, normalize service key order, and write the correspondingly named YAML
file with source provenance.
"""
...
def render_compose(source: Path) -> str:
    ...
    return ensure_service_spacing(buffer.getvalue())
...
def ensure_service_spacing(text: str) -> str:
    before, marker, after = text.partition("services:\n")
    if not marker:
        return text
    ...
    return before + marker + services + "".join(remainder)
...
def generated_header(source: Path, compose: CommentedMap) -> str:
    ...
    lines = [
        ...
        f"# based on `{source.name}` with resolved includes",
    ]
...
def remote_files(root_dir: Path) -> list[Path]:
    ignored = {".git", "utils"}
    return sorted(
        path for path in root_dir.rglob(f".*{REMOTE_SUFFIX}")
...
def render_files(root_dir: Path, dry_run: bool) -> None:
    for source in remote_files(root_dir):
        output_name = source.name.removeprefix(".").removesuffix(REMOTE_SUFFIX) + ".yml"
